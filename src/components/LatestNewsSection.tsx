...
export const LatestNewsSection: React.FC = () => {
  ...
  const headlineNews = useMemo(() => {
    return LATEST_NEWS_DATA.find((item) => item.featured) || LATEST_NEWS_DATA[0];
  }, []);

  const supportingNews = useMemo(() => {
    return LATEST_NEWS_DATA.filter((item) => item.id !== headlineNews.id).slice(0, 4);
  }, [headlineNews]);

  // Categories list for Archive
  const categories = useMemo(() => {
    const cats = Array.from(new Set(LATEST_NEWS_DATA.map((item) => item.category)));
    return ['Semua', ...cats];
  }, []);

  // Filtered news for Archive modal
  const filteredArchiveNews = useMemo(() => {
    return LATEST_NEWS_DATA.filter((item) => {
  ...
  return (
    <section id="berita" className="py-16 sm:py-24 bg-[#fbf7ee] relative overflow-hidden">
      {/* Background Decorative Islamic Lattice Watermark */}
      <div className="absolute inset-0 bg-islamic-hero-rich opacity-40 pointer-events-none z-0" />
      ...
      <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 sm:mb-12 pb-6 border-b border-[#e5dfd3]">
          <div>
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#0a3622]/8 border border-[#c87a1e]/25 text-[#0a3622] text-xs font-semibold uppercase tracking-wider mb-3 shadow-2xs">
            ...
            <p className="mt-2 font-worksans text-sm sm:text-base text-[#414943] leading-relaxed max-w-xl">
              Ikuti dokumentasi aktivitas pembinaan akhlak, capaian prestasi, dan syiar keislaman santri MI Asih Putera.
            </p>
          </div>
                  ...
                  <div className="flex-1 flex flex-col justify-between min-w-0 py-0.5">
                    <div>
                      <div className="flex items-center gap-1 text-[10.5px] font-manrope text-[#717972] mb-0.5">
                        ...
                        <span className="truncate">{news.date.split('/')[1] || news.date}</span>
                      </div>
                    ...
                    <div className="pt-1.5 flex items-center justify-between text-xs font-manrope font-semibold text-[#0a3622] group-hover:text-[#c87a1e]">
                      <span className="text-[10.5px] text-[#717972] font-normal">{news.readTime}</span>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-[#fcfaf5] rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-[#e8e4dc] animate-in zoom-in-95 my-6 flex flex-col">
            ...
            <div className="p-6 sm:p-7 bg-[#0a3622] text-white rounded-t-2xl relative overflow-hidden">
              <div className="absolute inset-0 bg-islamic-hero-rich opacity-25 pointer-events-none" />
              <div className="relative z-10 flex items-start justify-between">
                <div>
                  ...
                  <p className="font-worksans text-xs sm:text-sm text-[#a4d1b4] mt-1">
                    Koleksi lengkap liputan kegiatan santri, prestasi, dan agenda resmi MI Asih Putera Cimahi.
                  </p>
                </div>
            ...
            <div className="p-5 sm:p-6 bg-white border-b border-[#e8e4dc] space-y-4">
              <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
                ...
                <div className="text-xs font-manrope font-semibold text-[#717972] self-end sm:self-center">
                  Menampilkan <span className="text-[#0a3622] font-bold">{filteredArchiveNews.length}</span> dari {LATEST_NEWS_DATA.length} artikel
                </div>
              </div>
            ...
            <div className="p-6 sm:p-7 flex-1 overflow-y-auto max-h-[55vh]">
              {filteredArchiveNews.length === 0 ? (
                <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-[#d6cfc0]">
                  ...
                  <p className="font-manrope font-bold text-base text-[#001f11]">Tidak ada berita yang sesuai</p>
                  <p className="font-worksans text-xs text-[#717972] mt-1">Coba gunakan kata kunci lain atau pilih kategori Semua.</p>
                </div>
              ) : (
